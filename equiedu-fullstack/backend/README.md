...
## Security hardening in v1.2

This revision validates active sessions on every authenticated request, rejects inactive users during login/refresh, detects rotated refresh-token reuse and revokes the token family/session, locks refresh-token rows on PostgreSQL during rotation, bounds request metadata, trusts forwarding headers only from configured proxy networks, improves login rate-limit accounting, revokes sessions after password changes, validates production secrets, adds dependency readiness checks, hardens the Docker image, and synchronizes missing database indexes.

> Run the test suite against PostgreSQL + Redis before production deployment; SQLite/FakeRedis tests cannot reproduce every concurrency/property of those services.
