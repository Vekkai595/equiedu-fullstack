...
class TokenService:
    ...
    def rotate_refresh_token(self, refresh_token: str, *, ip_address: str, user_agent: str) -> TokenPairResponse:
        ...
        record = self.refresh_tokens.get_by_jti(payload.jti, for_update=True)
        ...
        if record.revoked_at is not None:
            # Reuse of a rotated token is treated as compromise: kill the whole family/session.
            if record.replaced_by_jti:
                self.refresh_tokens.revoke_family(record.token_family)
                session = self.sessions.get_by_id(record.session_id)
                if session and session.revoked_at is None:
                    self.sessions.revoke(session)
                self.refresh_tokens.db.commit()
            raise InvalidRefreshTokenError('Refresh token reuse detected')
        if record.user_id != int(payload.sub) or record.session_id != payload.sid:
            raise InvalidRefreshTokenError('Refresh token claims do not match server state')
