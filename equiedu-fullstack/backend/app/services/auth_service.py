...
from app.core.rate_limit import RateLimitExceeded, ensure_login_allowed, record_login_failure, clear_login_attempts
...
class AuthService:
    ...
    def login(self, payload: LoginRequest, meta: dict[str, str]) -> TokenPairResponse:
        ...
        try:
            ensure_login_allowed(key_ip)
            ensure_login_allowed(key_identifier)
        ...
        if user is None or not user.is_active or not verify_password(payload.password, user.password_hash):
            record_login_failure(key_ip)
            record_login_failure(key_identifier)
        ...
        # Do not clear the shared IP bucket on success; another account may be under attack.
    ...
    def refresh(self, refresh_token: str, meta: dict[str, str]) -> TokenPairResponse:
        ...
        if user is None or not user.is_active:
            raise InvalidRefreshTokenError('Refresh token user is inactive')
