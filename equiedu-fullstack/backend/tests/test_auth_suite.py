...
def test_revoked_session_invalidates_access_token(client):
    register(client)
    tokens = login(client).json()
    session_id = client.get('/api/v1/sessions', headers=auth_header(tokens['access_token'])).json()['items'][0]['id']
    assert client.delete(f'/api/v1/sessions/{session_id}', headers=auth_header(tokens['access_token'])).status_code == 200
    assert client.get('/api/v1/users/me', headers=auth_header(tokens['access_token'])).status_code == 401

def test_inactive_user_cannot_login_or_refresh(client, db_session):
    register(client)
    tokens = login(client).json()
    db=db_session(); user=db.query(User).filter_by(email='user@example.com').first(); user.is_active=False; db.commit(); db.close()
    assert login(client).status_code == 401
    assert client.post('/api/v1/auth/refresh', json={'refresh_token':tokens['refresh_token']}).status_code == 401

def test_refresh_reuse_revokes_session(client):
    register(client)
    ...
    assert client.get('/api/v1/users/me',headers=auth_header(second['access_token'])).status_code == 401

def test_update_username_uses_registration_rules(client):
    register(client); tokens=login(client).json()
    res=client.patch('/api/v1/users/me',json={'username':'bad space'},headers=auth_header(tokens['access_token']))
    assert res.status_code == 422

def test_oversized_device_id_is_bounded(client):
    register(client)
    res=client.post('/api/v1/auth/login',json={'identifier':'user@example.com','password':'StrongPassword123'},headers={'X-Device-ID':'x'*10000})
    assert res.status_code == 200

def test_change_password_revokes_existing_session(client):
    register(client); tokens=login(client).json()
    res=client.patch('/api/v1/users/me/password',json={'current_password':'StrongPassword123','new_password':'EvenStronger123'},headers=auth_header(tokens['access_token']))
    assert res.status_code == 200
    assert client.get('/api/v1/users/me',headers=auth_header(tokens['access_token'])).status_code == 401
